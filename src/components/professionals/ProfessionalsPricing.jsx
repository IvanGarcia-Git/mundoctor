...
const PricingCard = ({ plan, index, billingCycle }) => {
  ...
  const calculateDiscount = () => {
    const originalPrice = billingCycle === 'annually' 
      ? parseFloat(plan.annualOriginalPrice?.replace('€', '')) 
      : parseFloat(plan.originalPrice?.replace('€', ''));
    const currentPrice = billingCycle === 'annually'
      ? parseFloat(plan.annualPrice?.replace('€', '') || plan.price.replace('€', ''))
  ...
  return (
    ...
    >
      ...
      {discount && (
        <div className="absolute top-6 right-6 bg-green-500/10 dark:bg-green-500/20 text-green-700 dark:text-green-300 text-xs font-bold px-2 py-1 rounded-full">
          Ahorra {discount}%
        </div>
      )}
      <div className="flex-grow">
        ...
        <div className="mb-1">
          <span className={`text-4xl font-bold ${plan.textColor}`}>
            {billingCycle === 'annually' ? plan.annualPrice || plan.price : plan.price}
          </span>
          {((billingCycle === 'annually' && plan.annualOriginalPrice) || (billingCycle === 'monthly' && plan.originalPrice)) && (
            <span className="text-sm text-muted-foreground dark:text-gray-400 line-through ml-1">
              {billingCycle === 'annually' ? plan.annualOriginalPrice : plan.originalPrice}
            </span>
          )}
          <span className={`text-muted-foreground dark:text-gray-400 ${plan.textColor === 'text-white' || plan.textColor.includes('dark:text-white') ? 'opacity-80' : ''}`}>
            {billingCycle === 'annually' ? '/año' : plan.period}
          </span>
          {billingCycle === 'annually' && (plan.annualPrice || plan.price !== '0€') && (
            <div className="text-sm text-green-600 dark:text-green-400 mt-1">
              Ahorras {plan.name === 'Básico' ? '289.90€' : '319.90€'} al año
            </div>
          )}
        </div>
          ...
          <div className="mt-8">
            <span className="isolate inline-flex rounded-lg shadow-sm bg-background dark:bg-gray-800 p-1">
              ...
              >
                ...
                <span className="ml-1.5 hidden sm:inline-block bg-green-500/20 dark:bg-green-500/30 text-green-600 dark:text-green-300 text-xs font-bold px-1.5 py-0.5 rounded-full">Ahorra hasta 320€/año</span>
              </button>
            </span>
