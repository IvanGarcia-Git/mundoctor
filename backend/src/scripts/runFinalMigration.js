...
async function runFinalMigration() {
  try {
    console.log('🚀 Running final Clerk ID migration...');
    ...
    // Run the migration
    console.log('🔄 Executing final migration...');
    const migrationPath = path.join(__dirname, '../../migrations/007_final_clerk_id_migration.sql');
    ...
    console.log(`✅ Primary key changed to clerk_id: ${primaryKey.rows[0]?.column_name === 'id' ? 'SUCCESS' : 'FAILED'}`);
