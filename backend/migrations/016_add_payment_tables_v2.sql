-- Migration 016: Add payment system tables (Version 2)
...
-- Create subscriptions table (no foreign key to users initially)
CREATE TABLE subscriptions (
  id VARCHAR(255) PRIMARY KEY,
  user_id VARCHAR(255) NOT NULL, -- Will reference users.id
  plan_id VARCHAR(255),
...
-- Create payments table (no foreign keys initially)
CREATE TABLE payments (
  id VARCHAR(255) PRIMARY KEY,
  user_id VARCHAR(255) NOT NULL, -- Will reference users.id
  subscription_id VARCHAR(255), -- Will reference subscriptions.id
...
-- Create invoices table (no foreign keys initially)
CREATE TABLE invoices (
  id VARCHAR(255) PRIMARY KEY,
  user_id VARCHAR(255) NOT NULL, -- Will reference users.id
  subscription_id VARCHAR(255), -- Will reference subscriptions.id
  payment_id VARCHAR(255), -- Will reference payments.id
...
-- Create transactions table (no foreign keys initially)
